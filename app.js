...
async function startVoiceCommandOnce() {
  ...
  setStatus("Listening… say: Start / Teach / Unlock / Lock / Reset", "info");
  speak("Listening. Say start, teach, unlock, lock, or reset.");
    ...
    rec.onresult = async (ev) => {
      ...
      try {
        if (has("start")) {
          ...
          speak("I did not catch a command. Try start, teach, unlock, lock, or reset.");
          setStatus("Try: Start / Teach / Unlock / Lock / Reset", "warn");
        }
      } finally {
        resolve();
      }
...
function setAppState(newState) {
  appState = newState;
  logLine("info", `State → ${newState.toUpperCase()}`, "STATE");
...
function lockApp() {
  ...
  speak("Locked.");
  logLine("info", "Manual lock", "STATE");
  addXp(2, "Locked manually");
  ...
  autoLockTimer = setInterval(async () => {
    ...
    if (!det) {
      ...
      if (noFaceCount >= AUTO_LOCK_SEC) {
        logLine("warn", `No face for ${AUTO_LOCK_SEC}s → auto-lock`, "STATE");
        setAppState("locked");
        setStatus("Auto-locked 🔒 (face gone)", "warn");
        speak("Auto locked. Face not detected.");
        if (window.mbIsConnected?.()) window.mbSendLine?.("LOCK");
      }
    } else {
      noFaceCount = 0;
...
async function startTestMode() {
  ...
  if (!enrolled) {
    setStatus("Teach your face first!", "error");
    speak("Teach your face first.");
    return;
  }
  ...
  if (btnTestMode) btnTestMode.textContent = "🧪 Test Mode: ON";
  logLine("success", "Test Mode activated — continuous scan", "TEST");
  speak("Test mode on. Continuous scanning.");
  ...
  testLoopId = setInterval(async () => {
    ...
    if (!det?.descriptor) {
      if (appState === "unlocked") {
        setAppState("locked");
        setStatus("TEST: Locked 🔒 (no face)", "warn");
        if (window.mbIsConnected?.()) window.mbSendLine?.("LOCK");
      }
      return;
    }
    ...
    if (isMatch && appState !== "unlocked") {
      ...
      setStatus(`TEST: Unlocked ✅ (d=${dist.toFixed(3)})`, "success");
      logLine("success", `TEST MATCH d=${dist.toFixed(3)}`, "TEST");
      if (window.mbIsConnected?.()) window.mbSendLine?.("MATCH");
    } else if (!isMatch && appState !== "locked") {
      setAppState("locked");
      setStatus(`TEST: Locked ❌ (d=${dist.toFixed(3)})`, "error");
      logLine("error", `TEST NO MATCH d=${dist.toFixed(3)}`, "TEST");
...
function stopTestMode() {
  ...
  if (btnTestMode) btnTestMode.textContent = "🧪 Test Mode";
  if (btnVerify) btnVerify.style.display = "";
  logLine("info", "Test Mode stopped", "TEST");
  speak("Test mode off.");
  setAppState("locked");
  setStatus("Test mode ended. Locked 🔒", "warn");
}
...
function showPinOverlay(mode) {
  ...
  if (pinTitle) {
    pinTitle.textContent = mode === "set"
      ? "🔑 Set a new 4-digit PIN"
      : "🔐 Enter PIN to unlock Test Mode";
  }
  updatePinDots();
...
async function handlePinSubmit() {
  ...
  if (pinMode === "set") {
    ...
    logLine("success", "PIN updated", "PIN");
    speak("PIN saved.");
    hidePinOverlay();
  ...
  if (pinBuffer === stored) {
    logLine("success", "PIN correct — access granted", "PIN");
    speak("Access granted.");
    addXp(15, "Lab access granted");
    hidePinOverlay();
    if (pinResolve) { pinResolve(true); pinResolve = null; }
  } else {
    logLine("warn", "Wrong PIN", "PIN");
    speak("Wrong pin.");
