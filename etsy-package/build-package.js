/**
 * Face Quest — Etsy Package Builder
 ...
 *
 * Produces:
 *   etsy-package/FaceQuest-v1.0.0/          (folder)
 *   etsy-package/FaceQuest-v1.0.0.zip       (final ZIP)
...
const ZIP_DIR = resolve(PKG, `FaceQuest-${VERSION}`);
...
async function main() {
  console.log(`\n🎮 Face Quest Etsy Package Builder — ${VERSION}\n`);
  ...
  const zipPath = join(PKG, `FaceQuest-${VERSION}.zip`);
  if (existsSync(zipPath)) rmSync(zipPath);
  let zipped = false;
  try { execSync(`cd "${PKG}" && zip -r "${zipPath}" "FaceQuest-${VERSION}/"`, { stdio: 'pipe' }); zipped = true; } catch {}
  if (!zipped) try { execSync(`cd "${PKG}" && ditto -c -k --keepParent "FaceQuest-${VERSION}" "${zipPath}"`, { stdio: 'pipe' }); zipped = true; } catch {}
