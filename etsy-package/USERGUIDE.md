...
the **Face Quest** listing on Etsy.
...
├── FaceQuest-v*/            📦 Build staging dir (rebuilt on demand)
├── FaceQuest-v*.zip         📦 Final ZIP (this is what the buyer downloads)
│
...
- **What:** Node script that renders the 6 printable HTML templates to
  ...
  everything into `FaceQuest-v<version>.zip`.
...
- **Outputs:**
  ...
  - `etsy-package/FaceQuest-v<version>/` (staging dir, rebuilt)
  - `etsy-package/FaceQuest-v<version>.zip` (the buyer ZIP)
...
- **What:** "We Teach Computers to See Us!" poster in 5 big kid-safe steps.
...
- **The seven mockups:**
  ...
  2. **16-cell feature grid** — 3 modes × features
...
11. Attach the digital file: `FaceQuest-v<version>.zip`.
...
17. Keep old ZIPs tracked in Git (the `.gitignore` exemption keeps
    `etsy-package/FaceQuest-v*.zip` in history) so you can always
